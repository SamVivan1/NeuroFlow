...
// Inisialisasi bus I2C menggunakan TwoWire (Wire utama ESP32)
...
// Konfigurasi Baterai (Voltage Divider 10k + 10k)
...
// Variabel Kontrol Jarak Jauh (MQTT)
...
// Mengubah fungsi Reconnect menjadi NON-BLOCKING (Tanpa Loop 'while' dan tanpa 'delay')
...
// Desain Layout OLED Baru yang Lebih Bersih & Profesional
void oledShowMainScreen(const char* netStatus, float battVolt, int battPct, int hr, float tremor, int stress, int avgBPM, int avgSpO2, const char* act) {
  if (!oledAvailable) return;
  display.clearDisplay();
  display.setTextColor(SSD1306_WHITE);

  // 1. HEADER ROW (Status Jaringan & Informasi Daya Baterai)
  display.setTextSize(1);
  display.setCursor(0, 0);
  display.print(netStatus); 
  
  char battBuf[16];
  snprintf(battBuf, sizeof(battBuf), "%d%% %.2fV", battPct, battVolt);
  ...
  // 2. DATA GRAFIK UTAMA (Sisi Kiri: Tanda Vital)
  // Heart Rate
  ...
  // 3. DATA TEKNIS PERANGKAT (Sisi Kanan: Aktivitas & Metrik Tremor)
  display.setTextSize(1);
  ...
  display.print(stress); display.print("%");
...
float readBatteryVoltage() {
  ...
  return (raw / (float)ADC_MAX_VALUE) * ADC_REFERENCE * VOLTAGE_DIVIDER_RATIO;
}
...
void jalankanKalibrasi() {
  ...
  oledPrintLines("Calibration Started", "Keep Device Still", "Hold tight...", "5 Seconds");
...
void setup() {
  ...
  // PENYELESAIAN CONFLICT BUS: Jalankan satu inisialisasi di kecepatan 400kHz (Fast Mode)
  // Kecepatan ini didukung secara penuh oleh MPU6050, MAX30102, dan SSD1306.
  ...
  // Koneksi MAX30102 ke bus I2C (Parameter disamakan ke objek I2C_BUS tunggal)
...
void loop() {
  ...
  // Handle MQTT Reconnection secara modern (Asinkron & Non-Blocking)
  ...
  if (currentTime - lastTelemetryTime >= TELEMETRY_INTERVAL) {
    ...
    // Kirim data via MQTT hanya jika status terhubung (mencegah lag internal stack)
    if (mqttClient.connected()) {
      String jsonPayload = "{";
    ...
    // Tampilkan data langsung pada layout grid teranyar
    oledShowMainScreen(networkLabel, batteryVolt, batteryPct, displayBPM, tremorIntensity, calculatedStress, avgBPM, avgSpO2, getActivityLabel());
  }
}