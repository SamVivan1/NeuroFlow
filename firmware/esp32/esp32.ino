...
#define LAST_DATA_HOLD_M    kajf kfdfafdaS    2000
...
const float BATTERY_CALIBRATION_FACTOR = 1.133; // Faktor kalibrasi untuk menyesuaikan pembacaan ADC dengan multimeter
...
void oledShowMainScreen(const char* netStatus, float battVolt, int battPct, int hr, float tremor, int stress) {
  ...
  // 2. HEART RATE SECTION (Center Display dengan Logika Interaktif)
  ...
  if (!fingerPresent) {
    // STATE 1: Sensor belum dipakai
    display.setTextSize(1);
    display.print("-> Pasang Gelang <-");
  } else if (!warmupDone) {
    // STATE 2: Deteksi kontak, mulai menghitung mundur 8 detik
    ...
    // STATE 3: Pengambilan data selesai, tampilkan BPM
    display.setTextSize(2);
    if (hr > 0) {
      display.print(hr);
      display.setTextSize(1);
  ...
  // 3. TREMOR & STRESS SECTION (Bersebelahan)
  display.setTextSize(1);
...
float readBatteryVoltage() {
  ...
  // Dikali dengan BATTERY_CALIBRATION_FACTOR untuk menyesuaikan dengan multimeter
...
void loop() {
  ...
  if (currentTime - lastTelemetryTime >= TELEMETRY_INTERVAL) {
    ...
    // Publikasi data telemetri yang utuh ke cloud 
    if (mqttClient.connected()) {
      String jsonPayload = "{";
    ...
    // Refresh OLED dengan Layout Minimalis yang baru
